...
Item {
    ...
    function _pageImageSize() {
        ...
        return Qt.size(Math.max(1, Math.round(ps.width * root.renderScale * Screen.devicePixelRatio)), Math.max(1, Math.round(ps.height * root.renderScale * Screen.devicePixelRatio)));
    }
    ...
    Grid {
        ...
        Repeater {
            ...
            delegate: Item {
                ...
                function tileKey() {
                    return root._cacheKey(root.currentFrame, root.renderScale, isWholePage ? -1 : tileRow, isWholePage ? -1 : tileCol);
                }

                function refresh() {
                    if (!root.document || root.document.status !== PdfDocument.Ready || root.currentFrame < 0 || root.currentFrame >= root.document.pageCount) {
                        tileImage.source = "";
                        if (isWholePage)
                    ...
                    if (pdfRender.hasCache(root.currentFrame, root.renderScale, 0, tRow, tCol)) {
                    ...
                    if (isWholePage) {
                        const closest = pdfRender.closestCacheKey(root.currentFrame, root.renderScale, 0);
                    ...
                    if (imgSize.width > 0 && imgSize.height > 0) {
                        const reqId = pdfRender.requestRender(root.currentFrame, root.renderScale, 0, imgSize, 0, tRow, tCol, root.tileCount);
                        if (reqId !== 0) {
                ...
                Connections {
                    ...
                    function onRenderFinished(reqId, page, scale, rotation) {
                        if (reqId !== tileDelegate.pendingRequestId)
                            return;
                        if (page !== root.currentFrame)
                            return;
                        if (Math.abs(scale - root.renderScale) > 0.0001)
                            return;
    ...
    function refreshAll() {
        if (root.document && root.document.status === PdfDocument.Ready && root.currentFrame >= 0) {
            root.status = Image.Loading;
        } else {
            root.status = Image.Null;
        }
