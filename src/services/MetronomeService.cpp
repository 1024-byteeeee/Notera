...
MetronomeService::MetronomeService(QObject* parent) : QObject(parent)
{
    QSettings settings;
    m_bpm = qBound(1, settings.value(QStringLiteral("metronome/bpm"), 120).toInt(), 500);
    m_beatsPerMeasure =
        qBound(1, settings.value(QStringLiteral("metronome/beatsPerMeasure"), 4).toInt(), 16);
...
MetronomeService::~MetronomeService() { stop(); }
