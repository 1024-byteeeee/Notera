...
constexpr auto PreviewableImageSuffixes = {"jpg", "jpeg", "png", "bmp",
                                           "gif", "webp", "tif", "tiff"};
...
bool isSupportedScoreFile(const QString& sourcePath)
{
    ...
    return suffix == QStringLiteral("pdf") ||
           std::any_of(PreviewableImageSuffixes.begin(), PreviewableImageSuffixes.end(),
                       [&](const auto* item) { return suffix == QLatin1String(item); });
}

bool isPreviewableImage(const QString& sourcePath)
{
    const auto suffix = canonicalSuffix(sourcePath);
    return std::any_of(PreviewableImageSuffixes.begin(), PreviewableImageSuffixes.end(),
                       [&](const auto* item) { return suffix == QLatin1String(item); });
}
