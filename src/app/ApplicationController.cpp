...
ApplicationController::ApplicationController(QObject* parent) : QObject(parent)
{
    ...
    m_animationsEnabled =
        settings.value(QStringLiteral("appearance/animationsEnabled"), true).toBool();
    ...
    m_defaultScrollSpeed =
        settings.value(QStringLiteral("reader/defaultScrollSpeed"), 15.0).toDouble();
...
bool ApplicationController::animationsEnabled() const { return m_animationsEnabled; }
...
QString ApplicationController::accentColor() const { return m_accentColor; }
...
QString ApplicationController::currentScoreTitle() const { return m_currentScoreTitle; }

QUrl ApplicationController::currentFileUrl() const { return m_currentFileUrl; }

QString ApplicationController::currentFileType() const { return m_currentFileType; }

QString ApplicationController::currentScoreId() const { return m_currentScoreId; }

QString ApplicationController::currentScoreFolderId() const { return m_currentScoreFolderId; }

int ApplicationController::currentScorePageCount() const { return m_currentScorePageCount; }

double ApplicationController::autoScrollSpeed() const { return m_autoScrollSpeed; }
...
double ApplicationController::defaultScrollSpeed() const { return m_defaultScrollSpeed; }
...
int ApplicationController::longPressDragMs() const { return m_longPressDragMs; }
...
void ApplicationController::openScore(const QString& scoreId, const QString& title,
                                      ...
                                      const int pageCount, const QString& folderId)
{
...
QString ApplicationController::currentPage() const { return m_currentPage; }
...
QString ApplicationController::libraryFilter() const { return m_libraryFilter; }
...
QString ApplicationController::dataDirectory() const { return AppDataPaths::root(); }
...
static bool copyDirectoryRecursively(const QString& src, const QString& dst, QString* error)
{
    ...
    QDir().mkpath(dst);
    const auto entries =
        srcDir.entryInfoList(QDir::NoDotAndDotDot | QDir::AllEntries | QDir::Hidden);
...
static bool zipDirectory(const QString& srcDir, const QString& zipPath, QString* error)
{
    ...
    {
        if (error)
            *error = QStringLiteral("无法创建备份压缩包");
        return false;
    }
    ...
    while (it.hasNext())
    {
        ...
        else
        {
            ...
            {
                if (error)
                    *error = QStringLiteral("无法读取文件：%1").arg(filePath);
                return false;
            }
    ...
    {
        if (error)
            *error = QStringLiteral("写入备份压缩包失败");
        return false;
    }
    return true;
}
...
static bool writeBackupManifest(const QString& backupRoot, const QString& sourceRoot,
                                QString* error)
{
...
QString ApplicationController::runExportBackup(const QUrl& destinationFile)
{
    ...
    if (!QDir(parentPath).exists())
        return QStringLiteral("目标文件夹不存在");
    ...
    const auto connectionName =
        QStringLiteral("notera_export_") + QUuid::createUuid().toString(QUuid::WithoutBraces);
    ...
    QString snapshotError;
    {
        ...
        {
            QSqlQuery query(database);
            const auto snapshotSql =
                QStringLiteral("VACUUM INTO '%1'").arg(escapedSqlString(snapshotPath));
            snapshotOk = query.exec(snapshotSql);
            if (!snapshotOk)
                snapshotError = query.lastError().text();
        }
        database.close();
    }
    QSqlDatabase::removeDatabase(connectionName);
...
void ApplicationController::startExportDatabaseBackup(const QUrl& destinationFile)
{
    ...
    if (!m_exportWatcher)
    {
        ...
        connect(m_exportWatcher, &QFutureWatcher<QString>::finished, this,
                [this]()
                {
    ...
    m_exportWatcher->setFuture(
        QtConcurrent::run([destinationFile]() { return runExportBackup(destinationFile); }));
}
...
QString ApplicationController::runImportBackup(const QUrl& backupFile)
{
    ...
    const auto stagedRoot = QDir(parent).filePath(
        QStringLiteral(".notera-restore-") + QUuid::createUuid().toString(QUuid::WithoutBraces));
...
void ApplicationController::startImportDatabaseBackup(const QUrl& backupFile)
{
    ...
    if (!m_importWatcher)
    {
        ...
        connect(m_importWatcher, &QFutureWatcher<QString>::finished, this,
                [this]()
                {
    ...
    m_importWatcher->setFuture(
        QtConcurrent::run([backupFile]() { return runImportBackup(backupFile); }));
}

bool ApplicationController::applyPendingBackupRestore(QString* error)
{
    QSettings settings;
    const auto stagedRoot =
        QDir::cleanPath(settings.value(QStringLiteral("storage/pendingBackupRestore")).toString());
    ...
    const auto currentRoot = QDir::cleanPath(AppDataPaths::root());
    const auto rollbackRoot = QDir(QFileInfo(currentRoot).absolutePath())
                                  .filePath(QStringLiteral(".notera-rollback-") +
                                            QUuid::createUuid().toString(QUuid::WithoutBraces));
    ...
    bool updated = false;
    {
        ...
        if (database.open() && database.transaction())
        {
            ...
            query.prepare(
                QStringLiteral("UPDATE scores SET thumbnail_path = REPLACE(thumbnail_path, ?, ?) "
...
void ApplicationController::requestRestart() { emit restartRequested(); }
...
QString ApplicationController::clearAllData(const QString& confirmation)
{
    ...
    if (clearRoot.isEmpty() || clearRoot == QDir::rootPath() || clearRoot == QDir::homePath())
    {
...
{
    QSettings settings;
    const auto clearRoot =
        QDir::cleanPath(settings.value(QStringLiteral("storage/pendingClearRoot")).toString());
    if (clearRoot.isEmpty())
        return true;
...
{
    QSettings settings;
    const auto cleanNewPath =
        QDir::cleanPath(settings.value(QStringLiteral("storage/pendingDataDirectory")).toString());
    if (cleanNewPath.isEmpty())
        return true;
    ...
    if (QFileInfo::exists(dbPath))
    {
        ...
        bool databaseUpdated = false;
        {
            ...
            if (db.open() && db.transaction())
            {
                QSqlQuery query(db);
                query.prepare(
                    QStringLiteral("UPDATE scores SET file_path = REPLACE(file_path, ?, ?)"));
