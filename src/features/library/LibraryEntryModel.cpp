...
LibraryEntryModel::LibraryEntryModel(QObject* parent) : QAbstractListModel(parent) {}
...
int LibraryEntryModel::count() const { return m_entries.size(); }
...
QVariant LibraryEntryModel::data(const QModelIndex& index, const int role) const
{
    ...
    case CreatedDateRole:
        return entry.createdAt.isValid()
                   ? entry.createdAt.toLocalTime().toString(QStringLiteral("yyyy-MM-dd"))
                   : QString{};
    case PageCountRole:
...
{
    return {{ItemTypeRole, "itemType"},   {ItemIdRole, "itemId"},
            {TitleRole, "title"},         {CreatedDateRole, "createdDate"},
            {PageCountRole, "pageCount"}, {ThumbnailPathRole, "thumbnailPath"},
            {FavoriteRole, "favorite"},   {FilePathRole, "filePath"},
            {FileTypeRole, "fileType"},   {TagsRole, "tags"}};
}
...
void LibraryEntryModel::replaceAll(const QVariantList& folders, const QList<Score>& scores)
{
    ...
    for (const auto& value : folders)
    {
        ...
        entries.append(
            {QStringLiteral("folder"),
             ...
             0,
             {},
             folder.value(QStringLiteral("favorite")).toBool(),
             {},
             {},
    ...
    {
        entries.append({QStringLiteral("score"), score.id, score.title, score.createdAt,
                        score.pageCount, score.thumbnailPath, score.favorite, score.filePath,
                        score.fileType, score.tags});
    }
...
bool LibraryEntryModel::updateEntryTags(const QString& itemId, const QStringList& tags)
{
    ...
    {
        if (m_entries.at(row).itemId != itemId)
            continue;
        if (m_entries.at(row).tags == tags)
            return true;
...
bool LibraryEntryModel::updateEntryTitle(const QString& itemId, const QString& title)
{
    for (int row = 0; row < m_entries.size(); ++row)
    {
        if (m_entries.at(row).itemId != itemId)
            continue;
        if (m_entries.at(row).title == title)
            return true;
...
bool LibraryEntryModel::updateEntryFavorite(const QString& itemId, bool favorite)
{
    for (int row = 0; row < m_entries.size(); ++row)
    {
        if (m_entries.at(row).itemId != itemId)
            continue;
        if (m_entries.at(row).favorite == favorite)
            return true;
...
bool LibraryEntryModel::updateEntryThumbnail(const QString& itemId, const QString& thumbnailPath)
{
    for (int row = 0; row < m_entries.size(); ++row)
    {
        if (m_entries.at(row).itemId != itemId)
            continue;
        if (m_entries.at(row).thumbnailPath == thumbnailPath)
            return true;
