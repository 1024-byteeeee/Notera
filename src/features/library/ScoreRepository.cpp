...
ScoreRepository::ScoreRepository(QSqlDatabase database) : m_database(std::move(database)) {}
...
void ScoreRepository::rollbackTransaction() { m_database.rollback(); }

QList<Score> ScoreRepository::listAtFolder(const QString& folderId, const QString& searchQuery,
                                           QString* error) const
{
    QSqlQuery query(m_database);
    const auto needle = QStringLiteral("%%1%").arg(searchQuery);
    const auto folderCondition =
        folderId.isEmpty() ? QStringLiteral("folder_id IS NULL") : QStringLiteral("folder_id = ?");
...
bool ScoreRepository::setItemFavorite(const QString& itemId, const bool favorite,
                                      QString* error) const
{
    const auto type = itemTypeById(itemId, error);
    if (type == QStringLiteral("score"))
        return setFavorite(itemId, favorite, error);
...
bool ScoreRepository::updateThumbnail(const QString& scoreId, const QString& thumbnailPath,
                                      QString* error) const
{
    QSqlQuery query(m_database);
    query.prepare(
        QStringLiteral("UPDATE scores SET thumbnail_path = ?, updated_at = ? WHERE id = ?"));
...
bool ScoreRepository::updateThumbnails(const QHash<QString, QString>& thumbnailPaths,
                                       QString* error)
{
...
bool ScoreRepository::setFolder(const QString& scoreId, const QString& folderId,
                                QString* error) const
{
    QSqlQuery query(m_database);
...
bool ScoreRepository::addTag(const QString& scoreId, const QString& tagId, QString* error) const
{
    QSqlQuery query(m_database);
    query.prepare(
        QStringLiteral("INSERT OR IGNORE INTO score_tags (score_id, tag_id) VALUES (?, ?)"));
    query.addBindValue(scoreId);
...
QVariantList ScoreRepository::scoreTags(const QString& scoreId, QString* error) const
{
    QSqlQuery query(m_database);
    query.prepare(
        QStringLiteral("SELECT tags.id, tags.name FROM score_tags JOIN tags ON tags.id = "
                       "score_tags.tag_id WHERE score_tags.score_id = ? ORDER BY tags.name"));
    query.addBindValue(scoreId);
    if (!query.exec())
    {
        *error = query.lastError().text();
...
QHash<QString, QVariantList> ScoreRepository::allScoreTags(QString* error) const
{
    ...
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral("SELECT score_tags.score_id, tags.id, tags.name "
...
QVariantList ScoreRepository::itemTags(const QString& itemId, QString* error) const
{
    ...
    const auto table = type == QStringLiteral("folder")  ? QStringLiteral("folder_tags")
                       : type == QStringLiteral("score") ? QStringLiteral("score_tags")
                                                         : QString{};
    const auto column =
        type == QStringLiteral("folder") ? QStringLiteral("folder_id") : QStringLiteral("score_id");
    if (table.isEmpty())
        return {};
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral("SELECT tags.id, tags.name FROM %1 JOIN tags ON tags.id = "
                                 "%1.tag_id WHERE %1.%2 = ? ORDER BY tags.name COLLATE NOCASE")
                      .arg(table, column));
    query.addBindValue(itemId);
    if (!query.exec())
    {
...
bool ScoreRepository::addItemTag(const QString& itemId, const QString& tagId, QString* error) const
{
    ...
    QSqlQuery query(m_database);
    query.prepare(
        QStringLiteral("INSERT OR IGNORE INTO %1 (%2, tag_id) VALUES (?, ?)").arg(table, column));
    query.addBindValue(itemId);
    query.addBindValue(tagId);
    if (query.exec())
        return true;
...
bool ScoreRepository::removeItemTag(const QString& itemId, const QString& tagId,
                                    QString* error) const
{
    const auto type = itemTypeById(itemId, error);
    const auto table = type == QStringLiteral("folder")  ? QStringLiteral("folder_tags")
...
QList<Score> ScoreRepository::listByFolder(const QString& folderId, const QString& searchQuery,
                                           QString* error) const
{
    QSqlQuery query(m_database);
    const auto needle = QStringLiteral("%%1%").arg(searchQuery);
...
QList<Score> ScoreRepository::listByTag(const QString& tagId, const QString& searchQuery,
                                        QString* error) const
{
    QSqlQuery query(m_database);
    const auto needle = QStringLiteral("%%1%").arg(searchQuery);
...
QVariantList ScoreRepository::recentFolders(const QString& searchQuery, QString* error) const
{
    ...
    {
        result.append(
            QVariantMap{{"id", query.value(0).toString()},
                        {"name", query.value(1).toString()},
...
QVariantList ScoreRepository::foldersByTag(const QString& tagId, const QString& searchQuery,
                                           QString* error) const
{
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral(R"(
...
QVariantList ScoreRepository::childFolders(const QString& parentId, const QString& searchQuery,
                                           QString* error) const
{
    QSqlQuery query(m_database);
    const auto condition =
        parentId.isEmpty() ? QStringLiteral("parent_id IS NULL") : QStringLiteral("parent_id = ?");
    const auto searchCondition =
        searchQuery.isEmpty() ? QString() : QStringLiteral(" AND name LIKE ?");
    query.prepare(QStringLiteral(R"(
        SELECT id, name, created_at, favorite FROM folders
        WHERE %1%2
...
QVariantList ScoreRepository::folderScoresRecursive(const QString& folderId, QString* error) const
{
    ...
    while (query.next())
    {
        result.append(QVariantMap{{"id", query.value(0)},
                                  {"filePath", query.value(1)},
...
QVariantList ScoreRepository::folderExportEntries(const QString& folderId, QString* error) const
{
    ...
    while (query.next())
    {
        result.append(QVariantMap{{QStringLiteral("relativePath"), query.value(0)},
                                  ...
                                  {QStringLiteral("filePath"), query.value(2)}});
    }
    return result;
}

bool ScoreRepository::createFolder(const QString& name, const QString& parentId,
                                   QString* error) const
{
    QSqlQuery query(m_database);
    query.prepare(QStringLiteral("INSERT INTO folders (id, name, created_at, updated_at, "
                                 "parent_id) VALUES (?, ?, ?, ?, ?)"));
...
QVariantList ScoreRepository::tags(QString* error) const
{
    ...
    {
        result.append(
            QVariantMap{{"id", query.value(0).toString()}, {"name", query.value(1).toString()}});
    }
    return result;
}
...
bool ScoreRepository::renameFolder(const QString& folderId, const QString& name,
                                   QString* error) const
{
    QSqlQuery query(m_database);
...
bool ScoreRepository::canMoveFolder(const QString& folderId, const QString& parentId,
                                    QString* error) const
{
    if (folderId.isEmpty() || folderId == parentId)
        return false;
    if (parentId.isEmpty())
        return true;
    QSqlQuery query(m_database);
...
bool ScoreRepository::moveFolder(const QString& folderId, const QString& parentId,
                                 QString* error) const
{
    if (!canMoveFolder(folderId, parentId, error))
    {
        if (error->isEmpty())
            *error = QStringLiteral("文件夹不能移动到自身或其子文件夹中");
        return false;
    }
    QSqlQuery query(m_database);
    ...
    *changedCount = 0;
    ...
    for (const auto& itemId : itemIds)
    {
        ...
        if (type.isEmpty())
        {
            m_database.rollback();
            if (error->isEmpty())
                *error = QStringLiteral("项目不存在或目标文件夹无效");
            return false;
        }
        const auto currentFolderId = type == QStringLiteral("score") ? scoreFolderId(itemId, error)
                                                                     : folderParent(itemId, error);
        if (!error->isEmpty())
        {
            m_database.rollback();
            return false;
...
bool ScoreRepository::setItemsFavorite(const QStringList& itemIds, const bool favorite,
                                       QString* error)
{
    if (!m_database.transaction())
    {
...
bool ScoreRepository::deleteFolder(const QString& folderId, QString* error)
{
    ...
    deleteScoresQuery.prepare(
        QStringLiteral("DELETE FROM scores WHERE folder_id IN (%1)")
            .arg(QStringList(folderIds.size(), QStringLiteral("?")).join(QStringLiteral(","))));
    ...
    deleteFoldersQuery.prepare(
        QStringLiteral("DELETE FROM folders WHERE id IN (%1)")
            .arg(QStringList(folderIds.size(), QStringLiteral("?")).join(QStringLiteral(","))));
    for (const auto& id : folderIds)
    {
