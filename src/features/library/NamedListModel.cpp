...
NamedListModel::NamedListModel(QObject* parent) : QAbstractListModel(parent) {}
...
int NamedListModel::count() const { return m_items.size(); }
...
QVariant NamedListModel::get(const int index) const
{
    ...
    return QVariantMap{{"itemId", item.id}, {"name", item.name}, {"parentId", item.parentId}};
}
